...
create index if not exists academic_reports_scholar_idx
  on public.academic_reports (scholar_id, submitted_at desc);
...
create policy "users can read their own academic reports"
  on public.academic_reports for select
  to authenticated
  using (auth.uid() = scholar_id);

create policy "users can create their own academic reports"
